...
class _MusicScreenState extends State<MusicScreen> {
  ...
  String? _videoId(String value) => YoutubePlayer.convertUrlToId(value.trim());
  ...
  void _selectVideo(int index, {bool autoPlay = true}) {
    ...
    if (id == null) return;
    controller?.dispose();
    final next = YoutubePlayerController(initialVideoId: id, flags: YoutubePlayerFlags(autoPlay: autoPlay, mute: false));
    setState(() { controller = next; currentIndex = index; });
  }

  void _next() => _selectVideo((currentIndex + 1) % links.length);
  void _previous() => _selectVideo((currentIndex - 1 + links.length) % links.length);
  ...
  Future<void> _remove(int index) async {
    ...
    if (mounted) {
      ...
      if (updated.isEmpty) { controller?.dispose(); setState(() => controller = null); }
      else _selectVideo(index.clamp(0, updated.length - 1).toInt(), autoPlay: false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      if (controller != null) ...[
        YoutubePlayer(controller: controller!, showVideoProgressIndicator: true, progressIndicatorColor: Colors.red),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          IconButton(onPressed: links.length > 1 ? _previous : null, icon: const Icon(Icons.skip_previous)),
          const Padding(padding: EdgeInsets.symmetric(horizontal: 18), child: Text('Use the player controls above')),
      ...
      const SizedBox(height: 8),
      const Text('This feature uses the official YouTube player API. It does not download, extract, cache, or redistribute audio.', style: TextStyle(fontSize: 12, color: Colors.black54)),
    ]);
  }

  @override
  void dispose() { controller?.dispose(); link.dispose(); super.dispose(); }
}
