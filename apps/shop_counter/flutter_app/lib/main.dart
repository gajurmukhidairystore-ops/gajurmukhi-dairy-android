...
Future<void> main() async { AppProfile.current = AppProfile.store; WidgetsFlutterBinding.ensureInitialized(); final supabaseEnabled = supabaseUrl.isNotEmpty && supabasePublishableKey.isNotEmpty; if (supabaseEnabled) { await Supabase.initialize(url: supabaseUrl, publishableKey: supabasePublishableKey); } final db = AppDatabase(); await db.init(); runApp(ChangeNotifierProvider(create: (_) => BusinessProvider(db)..bootstrap(), child: GajurmukhiApp(supabaseEnabled: supabaseEnabled))); }
